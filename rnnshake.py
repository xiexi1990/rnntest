...
if True:
    ...
    history = model.fit(dataset, epochs=10, callbacks=[checkpoint_callback, tensorboard_callback])
else:

    model = build_model(len(vocab), embedding_dim, rnn_units, batch_size=1)
    model.load_weights(tf.train.latest_checkpoint('./training_checkpoints'))
    model.build(tf.TensorShape([1, seq_length]))
    model.summary()
...
def generate_text(model, start_string):
   ...
   #     predictions = tf.squeeze(predictions, 0)
        ...
        input_eval2 = tf.cast(tf.expand_dims(predicted_id, 0), tf.int32)

        input_eval3 = tf.cast(predicted_id, tf.int32)
        input_eval = tf.expand_dims(tf.concat([input_eval[0][1:5], input_eval3], 0),0)
        text_generated.append(idx2char[predicted_id[0]])

    return (start_string + ''.join(text_generated))

a = generate_text(model, start_string=u"ROMEO")
print(a)